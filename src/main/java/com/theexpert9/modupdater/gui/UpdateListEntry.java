...
public class UpdateListEntry extends ObjectSelectionList.Entry<UpdateListEntry> {
    ...
    public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean isHovered, float partialTick) {
        ...
        // BRAND NEW: Self-healing check. If tracker says completed but file is missing, force reset to idle!
        boolean filePhysicalCheck = java.nio.file.Files.exists(com.theexpert9.modupdater.util.DownloadManager.getPendingUpdatesDir().resolve(this.newFilename));
        if (currentState == 2 && !filePhysicalCheck) {
            neelesh.easy_install.util.GlobalDownloadTracker.setState(this.projectId, 0);
            currentState = 0; // Update local variable for the immediate frame draw
        }
